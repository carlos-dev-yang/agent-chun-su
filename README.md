...
Configure `executor.kind` as `codex`, `executor.path` as the discovered absolute executable path, and `executor.model` as `gpt-5.5`. The verified task/review adapter combination is Codex 0.153.4 with GPT-5.5. On macOS/arm64, CLI and Telegram reception also support Codex 0.154.0-alpha.6.2 with GPT-5.5; see the [role-specific compatibility record](docs/validation/TELEGRAM_CODEX_COMPATIBILITY_2026-09-13.md). Other combinations need boundary revalidation. Account sign-in belongs to Codex; Chun-su does not copy its credentials. `run JOB_ID` executes one attempt, while `worker` owns the queue in the foreground. `cancel`, `retry` and `resolve JOB_ID ANSWER` preserve the earlier attempt. Queueing and cancellation reach the current owner through a private local socket.
