...
구현했다. 이후 자연어 AI 대화를 기본으로 연결했다. [현재 AI 실행 검증](../validation/AI_CHAT_2026-09-11.md)을 확인한다.
...
별도 데이터 경로는 `--home`으로 선택한다. AI 대화는 기존 `executor.kind/path/model`과
Codex 로그인을 사용한다. 실행기가 없는 새 환경은 `chat --guided`로 설정할 수 있다.

기본 `chat`은 **AI 자연어 대화**다. “회의 안건 정리해줘”, “방금 문구를 줄여줘”처럼
업무 맥락을 이어서 요청할 수 있다. “현재 상태 확인해줘”, “보존된 작업 목록 보여줘”,
“텔레그램 설정 자료 설치해줘”는 호스트가 지원 범위를 검증한 뒤 실제 처리한다.
외부 전송처럼 지원하지 않는 요청은 초안 작성 등 가능한 부분과 추가 연동이 필요한
부분을 설명한다. AI가 임의 shell·패키지·MCP를 실행하지 않는다.

대화와 선택적으로 조회한 작업 ID·종류·상태는 설정된 실행기로 전달된다. 보고서 본문은
로컬 터미널에만 표시하고 AI에게 보내지 않는다. 대화 원문은 앱 기록에 저장하지 않고
실행기·Skill·스키마·입출력 해시와 호스트 처리 결과 해시를 남긴다. `/새대화`로 맥락을
초기화한다. 답변 중 새 입력을 하면 이전 생성을 취소하고 새 요청을 반영한다.

AI 없이 쓸 수 있는 `chat --guided`는 기존의 고정 문답형 설정이다. Gmail 인증에
필요한 계정·파일 경로 질문은 AI 대화에서도 이 로컬 단계로 인계한다.
