...
후속 실제 확인: 사용자 등록 Keychain 토큰으로 계정·보드 설정·본인 할당 이슈의
수동 API 조회가 성공했다. TODO 열의 실제 status는 `Backlog` / `10084`이며,
본인 TODO 23건 중 22건에 기한이 없다.
[수동 접근 검증 기록](../validation/PHASE_06_JIRA_LIVE_ACCESS.md)이 아래의
‘미조회·인증 미설정’ 준비 시점 기록을 갱신한다. 앱 자체의 연결 기능은 아직 없다.
