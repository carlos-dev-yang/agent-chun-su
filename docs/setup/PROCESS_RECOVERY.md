...
없어 워커 시작이 거부될 수 있다. `worker_not_configured`는 재시작만으로 해소되지
않는다. `/worker config`에서 현재 설정과 등록된 원본을 확인하고 다음처럼 처음
한 번 선택한다. 저장과 시작은 별도 조작이며 대화용 설정을 자동 선택하지 않는다.
...
검토용 설정을 선택하려면 `use review`, 모델만 바꾸려면 `model MODEL`을 사용한다.
변경은 워커가 중지돼 있고 진행 중 업무·설정 작업이 없을 때 컨트롤러가 저장한다.
경로·인증 정보는 채팅에 노출하지 않는다. 새 실행 경로·계정·환경은 기존 로컬
`config route task` 설정 경로를 사용한다. 실제 task 실행 설정이 바뀌면 기존 자료
공개 승인을 해제하며 원본의 승인을 복사하지 않는다. 같은 설정 재선택은 무변경이다.

저장한 업무용 설정은 원본의 나중 변경을 추종하지 않는다. 워커 재시작은 저장된
설정으로 실행을 다시 준비한다. 컨트롤러 재시작은 기존 워커 실행·중지 의도도
유지한다. 기존 대화용·검토용 유효 설정은 보존한다.
자연어 설정 조회·변경도 같은 경로를 사용하며 모델은 사용자가 명시해야 한다.
자연어 재시작은 restart_worker 하나로 요청한다. 오류·불확실한 저장은 자동 재시도하지
않으며 `/worker config`와 `/worker status`로 확인한다. CLI에서는 slash를 제거한
`chunsu worker config ...`를 사용한다.

설정 저장과 실행 환경 검증은 구분한다. 대화에 사용 가능한 실행기 버전이 업무용
실행 검증까지 통과한 것은 아닐 수 있다. 시작 시 prerequisites 오류가 나면
`chunsu doctor`의 task 항목에서 지원 버전·모델을 확인한다. 이번 변경은 기존
버전·모델 검증을 우회하지 않는다.
