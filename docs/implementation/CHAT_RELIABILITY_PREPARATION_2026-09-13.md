...
후속 구현: 이후 사용자 승인으로 주요 흐름 관측·로컬 경보·독립 감시 서비스를
구현하고 현재 Mac에 등록했다. 실제 완료 범위는 아래 작업표와
[운영 감시 검증](../validation/OPERATIONS_MONITOR_2026-09-13.md)에 기록했다.
사용법은 [운영 감시 실행 안내](../setup/OPERATIONS_MONITOR.md)를 따른다.
이 문서의 최초 준비 관측과 후속 구현 증거를 구분한다.
...
| CHAT-OBS-01 | 생존·준비·요청 결과를 구분하는 관측값과 상태 표시 | 조용한 정상 상태·연결 대기·AI 불가·컨트롤러 불가·활성 요청 정체 구분, 불완전한 과거 기록 표시 | 기본 관측 구현·검증 완료; 일반 AI 답변의 정확한 전송 시각 확장은 남음 |
| CHAT-OBS-02 | macOS 독립 감독자 감시와 기존 수신 복구의 통합 | 별도 감시 프로세스와 OS 등록, 명시적 중지 유지, 독립 잠금, 기존 수신기 유지 | 독립 관측 서비스 구현·현재 Mac 등록 완료; 감독자 정체 자동 복구와 Linux native 실행 검증은 별도 |
| CHAT-OBS-03 | 중단 요청 안내의 전달 계약과 구현 | 채택한 경우 원 작업 재실행 없이 안내 1회, 안내 전송 불확실도 보존, 반복 재시작 시 중복 없음 | 제안, 사용자 전달 정책 결정 필요 |
| CHAT-OBS-04 | 오류·정체 상황의 실제 동작 점검 | 감시 관측·경보·잠금·서비스 수명주기 확인; 실제 수신기의 AI 오류·네트워크 장애·복구까지는 별도 범위 | 분리된 홈의 감시 검증 완료; 실제 봇 장애 주입과 전체 응답 복구 수용 검증은 남음 |
